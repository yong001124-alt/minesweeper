...
**参考来源**：Neko Station（Steam 商店截图 `ss_*.jpg`，版权素材**仅存本地不入库**，已加入 .gitignore）
...
- [x] 教学引导角色「咪咪」插图：教学提示框头像 = 像素猫 `SPR.catSit`（scale 1.7）+「咪咪」名牌；咪咪即玩家猫自己（v2 第一人称叙事下提示是猫的内心独白，不引入第三方角色）
